...
def export_to_csv():
    with open(f'~/{user_name.get()}.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Name', 'Sex', 'Age', 'MOSFET_PIN', 'Warm', 'Painful'])
        for pin, warm, painful in zip(MOSFET_PINS, warm_vars, painful_vars):
            writer.writerow([user_name.get(), user_sex.get(), user_age.get(), pin, warm.get(), painful.get()])
...
def warm():
    if current_mosfet is None:
        ...
        warm_var.set(f"WARM : {warm_temp:.2f} °C")
...
def painful():
    if current_mosfet is None:
        ...
        painful_var.set(f"PAINFUL : {painful_temp:.2f} °C")
#     stop_heating()
...
frame_user.grid(row=3, column=0, pady=(10, 0))

tk.Label(frame_user, text="Name:").grid(row=0, column=0)
tk.Entry(frame_user, textvariable=user_name).grid(row=0, column=1)

tk.Label(frame_user, text="Sex:").grid(row=1, column=0)
tk.OptionMenu(frame_user, user_sex, "Male", "Female", "Other").grid(row=1, column=1)

tk.Label(frame_user, text="Age:").grid(row=2, column=0)
tk.Entry(frame_user, textvariable=user_age).grid(row=2, column=1)
...
button_export = tk.Button(frame_exit, text="Export", width=10, height=2, bg='green', fg='white', command=export_to_csv)
button_export.pack(side='right')

frame_stop = tk.Frame(window)
frame_stop.grid(row=5, column=0, pady=10)
button_stop = tk.Button(frame_stop, text="Stop", width=15, height=5, bg='red', fg='white', command=ar)
